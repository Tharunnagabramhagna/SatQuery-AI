export interface ReportData {
  ...
  confidenceScore?: {
    overall: number;
    label?: string;
    breakdown: Array<{ id: string; label: string; value: number; isDemo: boolean }>;
  };
  answer: string;
  evidence: string[];
  evidenceItems?: Array<{
...
export function exportToTxt(data: ReportData): void {
  ...
  lines.push(divider);
  lines.push(`Report Status    : DEMO REPORT (Mock Calibrated)`);
  ...
  if (data.confidenceScore?.breakdown) {
    ...
    data.confidenceScore.breakdown.forEach((item) => {
      lines.push(`    • ${item.label.padEnd(24)}: ${item.value}% (Demo)`);
    });
  }
  lines.push('');
  if (data.statistics?.temporalRange) {
    lines.push(`Timeline / Temporal Baseline:`);
    lines.push(`  ${data.statistics.temporalRange.t0} -> ${data.statistics.temporalRange.t1} (Demo)`);
    lines.push('');
  }
  ...
  if (data.statistics) {
    lines.push(subDivider);
    lines.push('3. STATISTICS & METRICS (DEMO)');
    lines.push(subDivider);
    lines.push(`  Area Changed     : ${data.statistics.areaChangedHectares} Hectares (Demo)`);
    lines.push(`  Buildings Detected: ${data.statistics.buildingCount} Footprints (Demo)`);
    lines.push(`  Vegetation Cover : ${data.statistics.vegetationCoverPercent}% (Demo)`);
    lines.push(`  Built-Up Area    : ${data.statistics.builtUpPercent}% (Demo)`);
    lines.push('');
  ...
  if (data.evidenceItems && data.evidenceItems.length > 0) {
    ...
    data.evidenceItems.forEach((item, idx) => {
      ...
      lines.push(`    Confidence      : ${item.confidence}% (Demo)`);
  ...
  lines.push(divider);
  lines.push('REPORT STATUS & NOTICE');
  lines.push(divider);
  lines.push('DEMO REPORT: Generated locally by SatQuery AI Space Intelligence Workspace.');
  lines.push('All geospatial inferences, sensor feeds, and confidence metrics are simulated');
  lines.push('for demonstration and evaluation purposes.');
  lines.push(divider);
...
export async function exportToPdf(data: ReportData): Promise<void> {
  ...
  const drawPageHeader = () => {
    ...
    doc.text('DEMO REPORT', pageWidth - marginX, 10, { align: 'right' });
  ...
  doc.text('DEMO REPORT', pageWidth - marginX - 22, currentY + 9.8, { align: 'center' });
  ...
  if (data.statistics?.temporalRange) {
    ...
    doc.setTextColor(100, 116, 139);
    doc.text(
      `Temporal Baseline: ${data.statistics.temporalRange.t0} → ${data.statistics.temporalRange.t1} (Demo)`,
      marginX + 75,
      currentY + 29
    );
  }
  ...
  // ─── Section 3: Statistics & Metrics Cards (Demo) ───
  if (data.statistics) {
    checkPageBreak(45);
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(10);
    doc.setTextColor(15, 23, 42);
    doc.text('3. QUANTITATIVE ANALYSIS METRICS (DEMO)', marginX, currentY);
    currentY += 5;
    ...
    doc.text('Land-Use Classification Distribution (Demo):', marginX, currentY);
    currentY += 4.5;
  ...
  if (data.evidenceItems && data.evidenceItems.length > 0) {
    ...
    data.evidenceItems.forEach((item, idx) => {
      ...
      const badgeText = `${item.type.toUpperCase()}  |  Confidence: ${item.confidence}% (Demo)${
  ...
  doc.setTextColor(148, 163, 184);
  doc.text(
    'Demonstration Notice: All spatial inferences and confidence indices are generated within the SatQuery AI sandbox for SIH26167 evaluation.',
    marginX,
    currentY
  );
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.setFontSize(7.5);
    doc.setTextColor(148, 163, 184);
    doc.text('SatQuery AI • Demo Report • SIH26167 Remote Sensing Intelligence', marginX, pageHeight - 8);
