 ...
 * Served via services/api.ts for complete backend compatibility.
 */

import type { DocumentationSection } from '../types';

export const MOCK_DOCUMENTATION_SECTIONS: DocumentationSection[] = [
  // ─── 1. Overview ──────────────────────────────────────────────────
  {
    id: 'overview',
 ...
 */
export function findDocumentationSection(id: string): DocumentationSection | undefined {
  return MOCK_DOCUMENTATION_SECTIONS.find((section) => section.id === id);
}
